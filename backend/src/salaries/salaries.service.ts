...
export class SalariesService {
  ...
  async create(data: {
    ...
    try {
      if (userId) {
        const periodStartStr = new Date(data.periodStart).toLocaleDateString(
          'en-US',
          { month: 'short', day: 'numeric', year: 'numeric' },
        );
        const periodEndStr = new Date(data.periodEnd).toLocaleDateString(
          'en-US',
          { month: 'short', day: 'numeric', year: 'numeric' },
        );
        await this.notificationsService.create({
          ...
          type: 'SALARY',
          actionUrl:
            data.employeeType === 'DOCTOR' ? '/doctor/salary' : '/nurse/salary',
