...
export class PatientsService {
    ...
    if (data.user) {
      ...
      return this.prisma.$transaction(async (tx) => {
          ...
          if (existingUser.role === UserRole.PATIENT) {
            ...
            if (existingPatient) {
              throw new ConflictException(
                `A patient with email ${data.user.email} already exists. Please register as returning patient instead.`,
              );
            }
          }
          // If user exists with different role, throw error
          throw new ConflictException(
            `An account with email ${data.user.email} already exists with role ${existingUser.role}. Please use a different email.`,
          );
        }
        ...
        if (patientData.dateOfBirth)
          cleanPatientData.dateOfBirth = new Date(patientData.dateOfBirth);
        if (patientData.bloodGroup)
          cleanPatientData.bloodGroup = patientData.bloodGroup.trim() || null;
        if (patientData.allergies)
          cleanPatientData.allergies = patientData.allergies.trim() || null;
        if (patientData.medicalHistory)
          cleanPatientData.medicalHistory =
            patientData.medicalHistory.trim() || null;
        cleanPatientData.currentState =
          patientData.currentState || 'IN_APPOINTMENT';
        if (patientData.departmentId)
          cleanPatientData.departmentId = patientData.departmentId;
  ...
  async getOverview(id: string) {
    ...
    const [appointments, bedAssignments, stateLogs, latestVitals] =
      await Promise.all([
  ...
  async updateByUserId(userId: string, data: any) {
    ...
    fieldsToUpdate.forEach((field) => {
      if (patientData[field] !== undefined) {
        updateData[field] =
          patientData[field] === '' ? null : patientData[field];
      }
    });
