...
export class DoctorsController {
  ...
  async getMyProfile(@Request() req) {
    ...
    if (!doctor) {
      ...
      console.log(
        `Doctor profile not found for user ${req.user.id}, creating it automatically...`,
      );
      try {
        doctor = await this.doctorsService.createDoctorProfile(req.user.id);
        if (!doctor) {
          throw new NotFoundException(
            'Doctor profile not found and could not be created. Please contact admin.',
          );
        }
      } catch (error: any) {
        console.error('Error creating doctor profile:', error);
        throw new NotFoundException(
          `Failed to create doctor profile: ${error.message || 'Unknown error'}`,
        );
      }
    }
    return doctor;
  ...
  async updateAvailability(
    ...
    if (body.doctorId && req.user.role === UserRole.ADMIN) {
      ...
      if (!doctor) {
        ...
        throw new NotFoundException(
          `Doctor not found with ID: ${body.doctorId}`,
        );
      }
      console.log('Found doctor:', doctor.id, 'Updating availability...');
      const result = await this.doctorsService.updateAvailability(
        doctor.id,
        body.availability,
      );
