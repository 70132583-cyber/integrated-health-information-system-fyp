    ...
    try {
      ...
      if (registerDto.role === 'ADMIN') {
        ...
        if (existingAdmin && existingAdmin.length > 0) {
          throw new Error(
            'Admin user already exists. Only one admin is allowed in the system.',
          );
        }
      }
      ...
      if (existingUser) {
        ...
        if (existingUser.isApproved) {
          throw new Error(
            'An account with this email already exists and is approved. Please login instead.',
          );
        }
        ...
        if (!existingUser.isApproved && !existingUser.profileCompleted) {
          console.log(
            `Removing existing unapproved user with email: ${registerDto.email} (profile not completed) to allow re-registration`,
          );
          try {
            ...
            console.log(
              `Successfully removed existing user: ${registerDto.email}`,
            );
            ...
            if (
              ...
              throw new Error(
                'Unable to remove existing account due to related data. Please contact admin for assistance.',
              );
            }
            throw new Error(
              deleteError.message ||
                'An account with this email already exists. Please contact admin if you need help.',
            );
          }
          ...
          throw new Error(
            'An account with this email already exists and is pending admin approval. Please wait for approval or contact support.',
          );
        }
      }
      ...
      const isApproved =
        registerDto.role === 'ADMIN'
          ? true
          : registerDto.isApproved !== undefined
