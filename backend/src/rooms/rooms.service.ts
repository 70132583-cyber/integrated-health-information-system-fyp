...
export class RoomsService {
  ...
  async assignBed(dto: AssignBedDto) {
    ...
    if (activeAssignment) {
      throw new BadRequestException(
        'Patient already has an active bed assignment',
      );
    }
  ...
  async createBed(roomId: string, dto: CreateBedDto) {
    ...
    if (existingBed) {
      throw new BadRequestException(
        `Bed with label "${dto.label}" already exists in this room`,
      );
    }
  ...
  async deleteBed(bedId: string) {
    ...
    if (bed.assignments && bed.assignments.length > 0) {
      throw new BadRequestException(
        'Cannot delete bed with active assignments. Please release the bed first.',
      );
    }
